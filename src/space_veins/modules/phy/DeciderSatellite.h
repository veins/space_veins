 ...
 * @see Decider80211p
 */
class SPACE_VEINS_API DeciderSatellite : public veins::BaseDecider {
public:
    enum DeciderSatelliteControlKinds {
        NOTHING = 22100,
        BITERROR, // the phy has recognized a bit error in the packet
        LAST_DECIDER_SATELLITE_CONTROL_KIND,
        RECWHILESEND
    };
...
protected:
    ...
    std::string myPath;
    DeciderSatelliteToSatellitePhyInterface* phySat;
...
protected:
     ...
     *
     */
    virtual DeciderSatelliteResult* checkIfSignalOk(veins::AirFrame* frame);

    simtime_t processNewSignal(veins::AirFrame* frame) override;
     ...
     */
    simtime_t processSignalEnd(veins::AirFrame* frame) override;
...
public:
     ...
     */
    DeciderSatellite(cComponent* owner, veins::DeciderToPhyInterface* phy, double minPowerLevel, double ccaThreshold, bool allowTxDuringRx, double centerFrequency, int myIndex = -1, bool collectCollisionStatistics = false)
        ...
        , notifyRxStart(false)
    {
        phySat = dynamic_cast<DeciderSatelliteToSatellitePhyInterface*>(phy);
        ASSERT(phySat);
    }
    ...
    bool cca(simtime_t_cref, veins::AirFrame*);
    int getSignalState(veins::AirFrame* frame) override;
    ~DeciderSatellite() override;
