...
Define_Module(BaseSatellitePhyLayer);

// --Initialization----------------------------------

template <class T>
T BaseSatellitePhyLayer::readPar(const char* parName, const T defaultValue)
{
    if (hasPar(parName))
        return par(parName);
    else
...
template int BaseSatellitePhyLayer::readPar<int>(const char* parName, const int);
template double BaseSatellitePhyLayer::readPar<double>(const char* parName, const double);
template simtime_t BaseSatellitePhyLayer::readPar<simtime_t>(const char* parName, const simtime_t);
template bool BaseSatellitePhyLayer::readPar<bool>(const char* parName, const bool);

void BaseSatellitePhyLayer::initialize(int stage)
{

    SatelliteChannelAccess::initialize(stage);
    ...
    if (stage == 0) {
        ...
        if (par("useNoiseFloor").boolValue()) {
            noiseFloorValue = veins::FWMath::dBm2mW(par("noiseFloor").doubleValue());
        }
        else {
            noiseFloorValue = 0;
        }
        minPowerLevel = par("minPowerLevel").doubleValue();
        minPowerLevel = veins::FWMath::dBm2mW(minPowerLevel);

        antennaGain_dBi = par("antennaGain_dBi").doubleValue();
        ...
        // if (scc->hasPar("sat") && (minPowerLevel - veins::FWMath::dBm2mW(scc->par("sat").doubleValue())) < -0.000001) {
...
unique_ptr<veins::Radio> BaseSatellitePhyLayer::initializeRadio()
{
    ...
    auto radio = veins::Radio::createNewRadio(recordStats, initialRadioState, initialRadioChannel, nbRadioChannels);

    //    - switch times to TX
    // if no RX to TX defined asume same time as sleep to TX
    radio->setSwitchTime(veins::Radio::RX, veins::Radio::TX, (hasPar("timeRXToTX") ? par("timeRXToTX") : par("timeSleepToTX")).doubleValue());
    // if no sleep to TX defined asume same time as RX to TX
    radio->setSwitchTime(veins::Radio::SLEEP, veins::Radio::TX, (hasPar("timeSleepToTX") ? par("timeSleepToTX") : par("timeRXToTX")).doubleValue());

    //    - switch times to RX
    // if no TX to RX defined asume same time as sleep to RX
    radio->setSwitchTime(veins::Radio::TX, veins::Radio::RX, (hasPar("timeTXToRX") ? par("timeTXToRX") : par("timeSleepToRX")).doubleValue());
    // if no sleep to RX defined asume same time as TX to RX
    radio->setSwitchTime(veins::Radio::SLEEP, veins::Radio::RX, (hasPar("timeSleepToRX") ? par("timeSleepToRX") : par("timeTXToRX")).doubleValue());

    //    - switch times to sleep
    // if no TX to sleep defined asume same time as RX to sleep
    radio->setSwitchTime(veins::Radio::TX, veins::Radio::SLEEP, (hasPar("timeTXToSleep") ? par("timeTXToSleep") : par("timeRXToSleep")).doubleValue());
    // if no RX to sleep defined asume same time as TX to sleep
    radio->setSwitchTime(veins::Radio::RX, veins::Radio::SLEEP, (hasPar("timeRXToSleep") ? par("timeRXToSleep") : par("timeTXToSleep")).doubleValue());

    return radio;
}

void BaseSatellitePhyLayer::getParametersFromXML(cXMLElement* xmlData, ParameterMap& outputMap)
{
...
void BaseSatellitePhyLayer::finish()
{
...
void BaseSatellitePhyLayer::initializeDecider(cXMLElement* xmlConfig)
{
    decider = nullptr;
...
unique_ptr<veins::Decider> BaseSatellitePhyLayer::getDeciderFromName(std::string name, ParameterMap& params)
{
    return nullptr;
}
...
void BaseSatellitePhyLayer::initializeAntenna(cXMLElement* xmlConfig)
{
    antenna = nullptr;
...
// 
    antenna = std::make_shared<SpaceXAntenna>(antennaGain_dBi);

    if (antenna == nullptr) {
        //throw cRuntimeError("Could not find an antenna with the name \"%s\".", name);
        throw cRuntimeError("Could not initialize SpaceXAntenna.");
    }
...
std::shared_ptr<veins::Antenna> BaseSatellitePhyLayer::getAntennaFromName(std::string name, ParameterMap& params)
{
    if (name == "SampledAntenna1D") {
        return initializeSampledAntenna1D(params);
    }

    return std::make_shared<veins::Antenna>();
}

std::shared_ptr<veins::Antenna> BaseSatellitePhyLayer::initializeSampledAntenna1D(ParameterMap& params)
{
    // get samples of the modeled antenna and put them in a vector
    ParameterMap::iterator it = params.find("samples");
    if (it == params.end()) {
        throw cRuntimeError("BaseSatellitePhyLayer::initializeSampledAntenna1D(): No samples specified for this antenna. \
    ...
    return std::make_shared<veins::SampledAntenna1D>(values, offsetType, offsetParams, rotationType, rotationParams, this->getRNG(0));
}

// -----AnalogueModels initialization----------------

void BaseSatellitePhyLayer::initializeAnalogueModels(cXMLElement* xmlConfig)
{
...
void BaseSatellitePhyLayer::handleMessage(cMessage* msg)
{
...
void BaseSatellitePhyLayer::handleAirFrame(AirFrame* frame)
{
...
void BaseSatellitePhyLayer::handleAirFrameStartReceive(AirFrame* frame)
{
    EV_TRACE << "Received new AirFrame " << frame << " at simulation time " << simTime() << "s." << endl;
    ...
    if (usePropagationDelay) {
        ...
        EV_TRACE << "Received new AirFrame " << frame << " with delay: " << delay << std::endl;
...
void BaseSatellitePhyLayer::handleAirFrameReceiving(AirFrame* frame)
{
...
void BaseSatellitePhyLayer::handleAirFrameEndReceive(AirFrame* frame)
{
...
void BaseSatellitePhyLayer::handleUpperMessage(cMessage* msg)
{
...
unique_ptr<AirFrame> BaseSatellitePhyLayer::createAirFrame(cPacket* macPkt)
{
    return space_veins::make_unique<AirFrame>(macPkt->getName(), AIR_FRAME);
}

unique_ptr<AirFrame> BaseSatellitePhyLayer::encapsMsg(cPacket* macPkt)
{
...
void BaseSatellitePhyLayer::handleUpperControlMessage(cMessage* msg)
{

    throw cRuntimeError("Received unknown control message from upper layer!");
}

void BaseSatellitePhyLayer::handleSelfMessage(cMessage* msg)
{
...
void BaseSatellitePhyLayer::sendControlMessageUp(cMessage* msg)
{
    send(msg, upperControlOut);
}

void BaseSatellitePhyLayer::sendMacPktUp(cMessage* pkt)
{
    send(pkt, upperLayerOut);
}

void BaseSatellitePhyLayer::sendMessageDown(AirFrame* msg)
{

    sendToChannel(msg);
}

void BaseSatellitePhyLayer::sendSelfMessage(cMessage* msg, simtime_t_cref time)
{
...
void BaseSatellitePhyLayer::filterSignal(AirFrame* frame)
{
    ASSERT(dynamic_cast<SatelliteChannelAccess* const>(frame->getArrivalModule()) == this);
    ASSERT(dynamic_cast<SatelliteChannelAccess* const>(frame->getSenderModule()));
    veins::Signal& signal = frame->getSignal();

    EV_DEBUG << "Center frequency's power level before antenna gains: " << signal.getAtCenterFrequency() << " mW." << std::endl;

    // Extract position and orientation of sender and receiver (this module) first
    const veins::AntennaPosition receiverPosition = antennaPosition;
    const veins::Coord receiverOrientation = antennaHeading.toCoord();
    // get POA from frame with the sender's position, orientation and antenna
    veins::POA& senderPOA = frame->getPoa();
    const veins::AntennaPosition senderPosition = senderPOA.pos;
    const veins::Coord senderOrientation = senderPOA.orientation;
    ...
    EV_DEBUG << "Center frequency's power level after antenna gains: " << signal.getAtCenterFrequency() << " mW." << std::endl;
    ...
    EV_DEBUG << "Center frequency's power level after analogue models which are *not* suitable for thresholding: " << signal.getAtCenterFrequency() << " mW." << std::endl;
}

// --Destruction--------------------------------

BaseSatellitePhyLayer::~BaseSatellitePhyLayer()
{
...
int BaseSatellitePhyLayer::getRadioState()
{
    Enter_Method_Silent();
    ASSERT(radio);
...
void BaseSatellitePhyLayer::finishRadioSwitching()
{
    radio->endSwitch(simTime());
    sendControlMsgToMac(new cMessage("Radio switching over", RADIO_SWITCHING_OVER));
}

simtime_t BaseSatellitePhyLayer::setRadioState(int rs)
{
    Enter_Method_Silent();
    ASSERT(radio);

    if (txOverTimer && txOverTimer->isScheduled()) {
        EV_WARN << "Switched radio while sending an AirFrame. The effects this would have on the transmission are not simulated by the BaseSatellitePhyLayer!";
    }
...
void BaseSatellitePhyLayer::setCurrentRadioChannel(int newRadioChannel)
{
    if (txOverTimer && txOverTimer->isScheduled()) {
        EV_WARN << "Switched channel while sending an AirFrame. The effects this would have on the transmission are not simulated by the BaseSatellitePhyLayer!";
    }
...
int BaseSatellitePhyLayer::getCurrentRadioChannel()
{
    return radio->getCurrentChannel();
}

int BaseSatellitePhyLayer::getNbRadioChannels()
{
...
void BaseSatellitePhyLayer::getChannelInfo(simtime_t_cref from, simtime_t_cref to, AirFrameVector& out)
{
    channelInfo.getAirFrames(from, to, out);
}

double BaseSatellitePhyLayer::getNoiseFloorValue()
{
    return noiseFloorValue;
}

void BaseSatellitePhyLayer::sendControlMsgToMac(cMessage* msg)
{
    sendControlMessageUp(msg);
}

void BaseSatellitePhyLayer::sendUp(AirFrame* frame, veins::DeciderResult* result)
{
    ...
    veins::PhyToMacControlInfo::setControlInfo(packet, result);

    sendMacPktUp(packet);
}

veins::BaseWorldUtility* BaseSatellitePhyLayer::getWorldUtility()
{
    return world;
}

void BaseSatellitePhyLayer::recordScalar(const char* name, double value, const char* unit)
{
