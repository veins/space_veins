...
//
//class veins::AirFrame;
//class veins::ChannelAccess;
//class veins::Radio;
...
class SPACE_VEINS_API BaseSatellitePhyLayer : public SatelliteChannelAccess, public veins::DeciderToPhyInterface, public veins::MacToPhyInterface {
...
protected:
    ...
    veins::ChannelInfo channelInfo; ///< Channel info keeps track of received AirFrames and provides information about currently active AirFrames at the channel.
    std::unique_ptr<veins::Radio> radio; ///< The state machine storing the current radio state (TX, RX, SLEEP).
     ...
     */
    std::shared_ptr<SpaceXAntenna> antenna;

    double antennaGain_dBi;

    std::unique_ptr<veins::Decider> decider; ///< Pointer to the decider module.

    /**
     * The analogue models to use which might attenuate or amplify a signal.
     */
    veins::AnalogueModelList analogueModels;
     ...
     */
    veins::AnalogueModelList analogueModelsThresholding;
    ...
    veins::BaseWorldUtility* world = nullptr; ///< Pointer to the World Utility, to obtain some global information
...
protected:
    ...
    /**
     * Read and return the projection string from the .net.xml file
     *
     * @param sumoNetXmlFile The .net.xml file name
     */
     ...
     */
    virtual std::unique_ptr<veins::Radio> initializeRadio();
     ...
     */
    virtual std::unique_ptr<veins::AnalogueModel> getAnalogueModelFromName(std::string name, ParameterMap& params)
    {
        return nullptr;
    }
     ...
     */
    virtual std::unique_ptr<veins::Decider> getDeciderFromName(std::string name, ParameterMap& params);
     ...
     */
    virtual std::shared_ptr<veins::Antenna> getAntennaFromName(std::string name, ParameterMap& params);
     ...
     */
    virtual std::shared_ptr<veins::Antenna> initializeSampledAntenna1D(ParameterMap& params);
     ...
     */
    virtual void handleAirFrame(veins::AirFrame* frame);
     ...
     */
    virtual void handleAirFrameStartReceive(veins::AirFrame* msg);

    /**
     * Handle incoming AirFrames with the state AirFrameState::receiving.
     */
    virtual void handleAirFrameReceiving(veins::AirFrame* msg);

    /**
     * Handle incoming AirFrames with the state AirFrameState::end_receive.
     */
    virtual void handleAirFrameEndReceive(veins::AirFrame* msg);
     ...
     */
    void sendMessageDown(veins::AirFrame* pkt);
     ...
     */
    virtual std::unique_ptr<veins::AirFrame> createAirFrame(cPacket* macPkt);

    /**
     * Create a signal corresponding to the received control information.
     */
    virtual void attachSignal(veins::AirFrame* airFrame, cObject* ctrlInfo)
    {
        throw cRuntimeError("Not implemented in the BaseSatellitePhyLayer. Override in subclass.");
    }

    /**
     * Encapsulate messages from the upper layer into an AirFrame and set all necessary attributes.
     */
    virtual std::unique_ptr<veins::AirFrame> encapsMsg(cPacket* msg);
     ...
     */
    virtual void filterSignal(veins::AirFrame* frame);
     ...
     */
    veins::Spectrum overallSpectrum;

public:
    ~BaseSatellitePhyLayer() override;
     ...
     */
    void sendUp(veins::AirFrame* packet, veins::DeciderResult* result) override;

    /**
     * Return a pointer to the simulations world-utility-module.
     */
    veins::BaseWorldUtility* getWorldUtility() override;
