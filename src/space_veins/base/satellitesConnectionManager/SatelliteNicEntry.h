 ...
 * @sa ConnectionManager, SatellitesConnectionManager
 */
class SPACE_VEINS_API SatelliteNicEntry : public veins::NicEntry {
public:
    /** @brief Points to this nics ChannelAccess module */
    SatelliteChannelAccess* satChAccess;

    /**
     * @brief Constructor, initializes all members
     */
    SatelliteNicEntry(cComponent* owner)
        : NicEntry(owner){};

    /**
     * @brief Destructor -- needs to be there...
     */
    virtual ~SatelliteNicEntry()
    {
    }
};
} // namespace space_veins
