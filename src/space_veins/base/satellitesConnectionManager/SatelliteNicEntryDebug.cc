...
void SatelliteNicEntryDebug::connectTo(NicEntry* other)
{
    EV_TRACE << "connecting nic #" << nicId << " and #" << other->nicId << endl;

    SatelliteNicEntryDebug* otherNic = (SatelliteNicEntryDebug*) other;
...
void SatelliteNicEntryDebug::disconnectFrom(NicEntry* other)
{
...
int SatelliteNicEntryDebug::collectGates(const char* pattern, GateStack& gates)
{
...
void SatelliteNicEntryDebug::collectFreeGates()
{
...
cGate* SatelliteNicEntryDebug::requestInGate(void)
{
    collectFreeGates();
    ...
    if (!freeInGates.empty()) {
        ...
        SatelliteChannelAccess* phyModule;
        // gate of the phy module
        cGate* phyGate;

        // to avoid unnecessary dynamic_casting we check for a "phy"-named submodule first
        if ((phyModule = dynamic_cast<SatelliteChannelAccess*>(nicPtr->getSubmodule("satellitePhy"))) == nullptr) phyModule = veins::FindModule<SatelliteChannelAccess*>::findSubModule(nicPtr);
...
cGate* SatelliteNicEntryDebug::requestOutGate(void)
{
    collectFreeGates();
