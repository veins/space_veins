...
void Radio::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        myReceptionState.setName("myReceptionState");
        if (simTime() >= getSimulation()->getWarmupPeriod()) {
            myStartTime = simTime();
        }else{
            myStartTime = getSimulation()->getWarmupPeriod();
        }
        lastBusy = simTime();
        WATCH(radioMode);
...
void Radio::finish()
{
    ...
    simtime_t totalLifeTime = simTime() - myStartTime;
    recordScalar("totalBusyTimeRatio", (statsTotalBusyTime.dbl() / totalLifeTime.dbl()));
}
